...
func (h *handler) List(c *fiber.Ctx) error {
	type JsonData struct {
		Filter md.VariantLocalFilter `json:"filter"`
	}
...
func (h *handler) Insert(c *fiber.Ctx) error {
	type jsonData struct {
		Data md.VariantLocal `json:"data"`
	}
	var payload jsonData
	...
	variantLocal := payload.Data
	result, status, err := Logic.Insert(variantLocal)
	if err != nil {
		return helper.JSON(c, err.Error(), status)
	}
...
func (h *handler) Update(c *fiber.Ctx) error {
	type JsonData struct {
		Filter md.VariantLocalFilter `json:"filter"`
		Data   md.VariantLocalUpdate `json:"data"`
	}
	var payload JsonData
	err := c.BodyParser(&payload)
	if err != nil {
	...
	variantLocal := payload.Data
	result, status, err := Logic.Update(filter, variantLocal)
	if err != nil {
		return helper.JSON(c, err.Error(), status)
	}
